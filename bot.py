...
def get_visibility_labels():
    labels = dict(BUTTON_LABELS)
    for index, button in enumerate(get_custom_buttons()):
        text = str(button.get("text", "")).strip()
        if text:
            labels[f"custom{index}"] = text
    return labels

def button_visibility_keyboard(mode):
    hidden = get_hidden_main_buttons()
    labels = get_visibility_labels()
    kb = InlineKeyboardMarkup(row_width=2)
    shown_count = 0
    for key, label in labels.items():
        ...
        shown_count += 1
    if shown_count == 0:
        kb.add(InlineKeyboardButton("No buttons available", callback_data="btnvis|noop|x"))
...
def hide_button_menu(m):
    bot.send_message(
        m.from_user.id,
        "🙈 Select a user-menu button to hide.\n\nThe change is saved in MongoDB and applies to all users.",
        reply_markup=button_visibility_keyboard("hide"),
    )
...
def show_button_menu(m):
    bot.send_message(
        m.from_user.id,
        "👁 Select a hidden user-menu button to show again.",
        reply_markup=button_visibility_keyboard("show"),
    )
...
def button_visibility_callback(c):
    ...
    if mode == "close":
        ...
        return
    if mode == "noop":
        bot.answer_callback_query(c.id, "No buttons available", True)
        return
    label = get_visibility_labels().get(key)
    if not label:
        bot.answer_callback_query(c.id, "Button no longer exists", True)
        return
    hidden = get_hidden_main_buttons()
    if mode == "hide":
        hidden.add(label)
        ...
        bot.answer_callback_query(c.id, "Invalid action", True)
        return
    ...
    try:
        bot.edit_message_reply_markup(
            c.from_user.id,
            c.message.message_id,
            reply_markup=button_visibility_keyboard(mode),
        )
    except Exception:
        pass
