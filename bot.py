...
def give_points_process(m):
    ...
    try:
        ...
        try:
            bot.send_message(
                user_id,
                f"💫 **You have gotten {points:,} points!**\n\n"
...
def button_name_step(m):
    try:
        ...
        state["text"] = text
        prompt = "Send the full URL:" if state["type"] == "link" else "Send the folder number:"
...
def button_data_step(m):
    try:
        ...
        if state["type"] == "link":
            if not data.startswith(("http://", "https://", "tg://")): raise ValueError("Invalid link")
        else:
...
def join_notification_keyboard():
    kb=InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("➕ Set Group",callback_data="joinnotify|set"),InlineKeyboardButton("🗑 Remove Group",callback_data="joinnotify|remove"),InlineKeyboardButton("📋 View Group",callback_data="joinnotify|view"))
    return kb

@bot.message_handler(func=lambda m:m.text=="👥 Join Notifications" and is_admin(m.from_user.id))
def join_notification_menu(m):
    bot.send_message(m.from_user.id,"👥 **New User Notification Group**\n\nThe bot sends full details here whenever a new user starts the bot. Make the bot admin in the group first.",reply_markup=join_notification_keyboard(),parse_mode="Markdown")
...
def join_notification_cb(c):
    ...
    try:
        if action=="view":
            group=get_config().get("join_notify_group") or "Not set";bot.send_message(c.from_user.id,f"Current notification group: `{group}`",parse_mode="Markdown");return bot.answer_callback_query(c.id)
        if action=="remove":
            set_config("join_notify_group",None);bot.send_message(c.from_user.id,"✅ Process Complete\nNotification group removed.",reply_markup=admin_menu());return bot.answer_callback_query(c.id,"Removed")
        msg=bot.send_message(c.from_user.id,"Send the group @username or numeric ID (`-100...`):",parse_mode="Markdown");bot.register_next_step_handler(msg,save_join_notification_group);bot.answer_callback_query(c.id,"Continue in chat")
    except Exception as exc:bot.send_message(c.from_user.id,f"❌ Process Failed\n{exc}")

def save_join_notification_group(m):
    try:
        value=(m.text or "").strip(); chat=bot.get_chat(value); member=bot.get_chat_member(chat.id,bot.get_me().id)
        if member.status not in ("administrator","creator"):raise ValueError("Bot must be admin in the group")
        set_config("join_notify_group",chat.id)
        bot.send_message(m.from_user.id,f"✅ Process Complete\nNotification group set: `{chat.id}`",parse_mode="Markdown",reply_markup=admin_menu())
        bot.send_message(chat.id,"✅ This group will now receive new-user notifications.")
...
def auto_target_step(m):
    try:
        state=_pending_auto.get(m.from_user.id);state["channel"]=(m.text or "").strip();bot.get_chat(state["channel"])
...
def save_auto_post(m):
    try:
        ...
        if not x:raise ValueError("Session expired")
        now=now_ts()
        ...
        auto_posts_col.insert_one({**x,'source_chat':m.chat.id,'source_message':m.message_id,'next_run':next_run,'active':True,'created_at':now})
        bot.send_message(m.from_user.id,"✅ Process Complete\nAuto post created successfully.",reply_markup=admin_menu())
    except Exception as exc:bot.send_message(m.from_user.id,f"❌ Process Failed\n{exc}",reply_markup=admin_menu())
...
def auto_import_keyboard():
    kb=InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("➕ Add Source",callback_data="importui|sourceadd"),InlineKeyboardButton("➖ Remove Source",callback_data="importui|sourceremove"),InlineKeyboardButton("📋 View Sources",callback_data="importui|sourcelist"),InlineKeyboardButton("📤 Import/Upload Method",callback_data="importui|method"))
    return kb
...
def import_ui_callback(c):
    ...
    try:
        ...
        if action=="sourceadd":
            msg=bot.send_message(c.from_user.id,"Send source chat @username or numeric ID:");bot.register_next_step_handler(msg,import_add_source_step);return bot.answer_callback_query(c.id,"Continue in chat")
        if action=="method":
...
def import_add_source_step(m):
    try:
        src=(m.text or "").strip();bot.get_chat(src);source_chats_col.update_one({'_id':src},{'$set':{'active':True,'added_at':now_ts()}},upsert=True);bot.send_message(m.from_user.id,f"✅ Process Complete\nSource added: {src}",reply_markup=admin_menu())
    except Exception as exc:bot.send_message(m.from_user.id,f"❌ Process Failed\n{exc}",reply_markup=admin_menu())
...
def import_method_step(m):
    try:
        ...
        if not state:raise ValueError("Session expired")
        name=((m.text or m.caption or 'Imported Method').strip().splitlines()[0][:100]);files=[{'chat':m.chat.id,'msg':m.message_id,'type':m.content_type}];number=fs.add(state['category'],name,files,state['price']);log_event('method_imported',m.from_user.id,number,{'name':name});bot.send_message(m.from_user.id,f"✅ Process Complete\nImported #{number}: {name}",reply_markup=admin_menu())
    except Exception as exc:bot.send_message(m.from_user.id,f"❌ Process Failed\n{exc}",reply_markup=admin_menu())
