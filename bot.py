...
def expire_restore_menu(m):
    ...
    rows = list(folders_col.find(query).sort([("cat",1),("name",1)]))
    kb = InlineKeyboardMarkup(row_width=1)
    for row in rows[:100]:
        kb.add(InlineKeyboardButton(f"{str(row.get('cat','')).upper()} • {row.get('name')}"[:62], callback_data=f"methodstatus|{mode}|{row['_id']}"))
    raw_bot.send_message(m.from_user.id, "Select a method:" if rows else "No methods available for this action.", reply_markup=kb if rows else None)

@bot.callback_query_handler(func=lambda c: c.data.startswith("methodstatus|"))
def method_status_cb(c):
    if not is_admin(c.from_user.id): return
    from bson import ObjectId
    ...
    if not row: return bot.answer_callback_query(c.id,"Not found",True)
    expired = mode == "expire"
    folders_col.update_one({"_id": row["_id"]}, {"$set": {"expired": expired, "updated_at": now_ts()}})
    send_method_notification("expired" if expired else "restored", {**row, "expired": expired})
    ...
    try:
        ...
        if part:
            ...
            if pending:
                ...
                raw_bot.send_message(m.chat.id, f"✅ File {part} added to pending #method{method_key}. Waiting for admin approval.")
                return
            ...
            if approved:
                ...
                raw_bot.send_message(m.chat.id, f"✅ Update file {part} is pending admin approval for {approved.get('name')}.")
                return
        ...
        raw_bot.send_message(m.chat.id, f"⏳ {name} has been {action}. It will appear in the bot after admin approval.")
        try:
            kb = InlineKeyboardMarkup(row_width=2)
            kb.add(
            ...
            raw_bot.send_message(ADMIN_ID, f"⏳ PENDING METHOD\n\n📁 Category: {cat.upper()}\n🏷 Name: {name}\n🏷 Source: #method{method_key}\n📎 Files: 1", reply_markup=kb)
        except Exception:
            pass
    except Exception as exc:
        try:
