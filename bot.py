...
def _queue_auto_import(m, edited=False):
    ...
    Supported:
      ...
      #method1.2 / #method1.3 as a reply -> attach replied text/media/file
    ...
    try:
        ...
        payload = _import_payload_message(m, part)
        file_item = {
        ...
        if part:
            ...
            if pending:
                ...
                if source_reply_enabled:
                    raw_bot.send_message(m.chat.id, f"✅ Part {part} added to #method{method_key}. Waiting for admin approval.")
                return
            ...
            if approved:
                ...
                if source_reply_enabled:
                    raw_bot.send_message(m.chat.id, f"✅ Part {part} queued for admin approval for {approved.get('name')}.")
                return

            if source_reply_enabled:
                raw_bot.send_message(m.chat.id, f"❌ Add the main #method{method_key} first, then add #method{method_key}.{part}.")
            return
        ...
        if len(lines) > 1 and lines[1].strip():
            ...
            name = (payload_lines[1] if len(payload_lines) > 1 else f"Method {method_key}")[:150]
