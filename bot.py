...
def admin_menu():
    ...
    kb.row("📊 Leaderboard", "🔎 Search")
    kb.row("📣 Auto Posts", "📥 Auto Import")
    kb.row("🧰 Folder Tools", "🧾 Logs")
    kb.row("💾 Backup/Export", "📨 Advanced Broadcast")
    kb.row("🎯 Advanced Points", "🎟 Promo Codes+")
    kb.row("❌ Exit")
...
def auto_post_commands(m):
    ...
    try:
        ...
        channel,mode,value=a[1],a[2],a[3]; _pending_auto[m.from_user.id]={'channel':channel,'schedule':mode,'value':value}; msg=bot.send_message(m.from_user.id,'Send/copy post content:'); bot.register_next_step_handler(msg,save_auto_post)
    except Exception as exc: bot.send_message(m.from_user.id,f"❌ {exc}")
...
def fallback(m):
    ...
    known = ["📂 FREE METHODS", "💎 VIP METHODS", "📦 PREMIUM APPS", "⚡ SERVICES", "💰 POINTS", "⭐ BUY VIP", "🎁 REFERRAL", "👤 ACCOUNT", "🆔 CHAT ID", "🏆 REDEEM", "📚 MY METHODS", "💎 GET POINTS", "⚙️ ADMIN PANEL", "🔎 Search", "📣 Auto Posts", "📥 Auto Import", "🧰 Folder Tools", "🧾 Logs", "💾 Backup/Export", "📨 Advanced Broadcast", "🎯 Advanced Points", "🎟 Promo Codes+"]
