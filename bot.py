...
def methods_list_cmd(m):
    ...
    for cat, title, icon in categories:
        ...
        for row in rows:
            ...
            lines.append(f"{pin}{icon} {row.get('name', 'Unnamed Method')}")
        sections.append("\n".join(lines))
...
def auto_import_channel_post(m):
    ...
    try:
        ...
        def _chat_id_equal(saved, current):
            if saved is None:
                return False
            try:
        ...
        cat = None
        if _chat_id_equal(cfg.get("auto_import_free_source"), m.chat.id):
            cat = "free"
        elif _chat_id_equal(cfg.get("auto_import_vip_source"), m.chat.id):
            cat = "vip"
        if not cat:
            return
        ...
        bot.send_message(m.chat.id, f"⏳ {name} has been {action}. It will appear in the bot after admin approval.")
        try:
            kb = InlineKeyboardMarkup(row_width=2)
            kb.add(
...
def _visibility_keyboard(mode):
    hidden = get_hidden_main_buttons()
    candidates = []
    for text in MAIN_MENU_BUTTONS:
        if (mode == "hide" and text not in hidden) or (mode == "show" and text in hidden):
            candidates.append(text)
    for item in get_custom_buttons():
        text = item.get("text")
        if text and ((mode == "hide" and text not in hidden) or (mode == "show" and text in hidden)):
            candidates.append(text)
    kb = InlineKeyboardMarkup(row_width=1)
    for i, text in enumerate(candidates):
        kb.add(InlineKeyboardButton(text, callback_data=f"visbtn|{mode}|{i}"))
    return kb, candidates

_visibility_sessions = {}
...
def visibility_menu(m):
    ...
    kb, items = _visibility_keyboard(mode)
    _visibility_sessions[(m.from_user.id, mode)] = items
    if not items:
        return admin_success(m.from_user.id, "No buttons available for this action")
    raw_bot.send_message(m.from_user.id, "Select a button to hide:" if mode == "hide" else "Select a button to show:", reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("visbtn|"))
def visibility_callback(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    try:
        _, mode, idx = c.data.split("|")
        items = _visibility_sessions.get((c.from_user.id, mode), [])
        text = items[int(idx)]
        hidden = list(get_config().get("hidden_main_buttons", []))
        if mode == "hide" and text not in hidden:
            hidden.append(text)
        elif mode == "show":
            hidden = [x for x in hidden if x != text]
        set_config("hidden_main_buttons", hidden)
        bot.answer_callback_query(c.id, "Updated", True)
        admin_success(c.from_user.id, f"Button {'hidden' if mode == 'hide' else 'shown'}: {text}")
    except Exception as exc:
        admin_error(c.from_user.id, exc)
...
def _notify_settings_keyboard():
    cfg = get_config()
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton(f"🔔 Method notifications: {'ON' if cfg.get('method_notify_enabled', True) else 'OFF'}", callback_data="notifyv2|toggle"))
