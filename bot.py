...
import os, time, random, string, threading, hashlib, hmac, json, csv, io, zipfile, traceback, logging, re
...
def sanitize_mongo_uri(uri):
    """Drop invalid/unsafe URI options while preserving the database and credentials."""
    try:
        parts = urlsplit(uri)
        allowed = {
            "retrywrites", "w", "wtimeoutms", "journal", "readpreference",
            "replicaset", "authsource", "tls", "ssl", "tlsallowinvalidcertificates",
            "connecttimeoutms", "sockettimeoutms", "serverselectiontimeoutms",
            "maxpoolsize", "minpoolsize", "appname", "directconnection",
            "compressors", "zlibcompressionlevel", "uuidrepresentation"
        }
        cleaned = [(k, v) for k, v in parse_qsl(parts.query, keep_blank_values=True) if k.lower() in allowed]
...
def admin_menu():
    ...
    kb.row("📸 Screenshot", "➕ Add Button")
    kb.row("➖ Remove Button", "🙈 Hide Button")
    kb.row("👁 Show Button", "➕ Add Channel")
    kb.row("➖ Remove Channel", "⚙️ Settings")
...
@bot.callback_query_handler(func=lambda c: c.data in ("gen_single", "gen_multi"))
def gen_codes_type(c):
    if not is_admin(c.from_user.id):
        bot.answer_callback_query(c.id, "Admin only", True)
        return
...
def process_code_points(m):
    ...
    try:
        ...
        if not session:
            bot.send_message(uid, "❌ Session expired. Open Generate Codes again.", reply_markup=admin_menu())
            return
        session["points"] = pts
...
def process_code_count(m):
    ...
    try:
        ...
        if session["multi"]:
            ...
            bot.send_message(uid, f"✅ {count} codes!\n\n<code>{chr(10).join(codes)}</code>", parse_mode="HTML", reply_markup=admin_menu())
...
def button_visibility_keyboard(mode):
    hidden = get_hidden_main_buttons()
    kb = InlineKeyboardMarkup(row_width=2)
    for key, label in BUTTON_LABELS.items():
        if mode == "hide" and label in hidden:
            continue
        if mode == "show" and label not in hidden:
            continue
        kb.add(InlineKeyboardButton(label, callback_data=f"btnvis|{mode}|{key}"))
    kb.add(InlineKeyboardButton("❌ Close", callback_data="btnvis|close|x"))
    return kb


@bot.message_handler(func=lambda m: m.text == "🙈 Hide Button" and is_admin(m.from_user.id))
def hide_button_menu(m):
    bot.send_message(m.from_user.id, "🙈 Select a user-menu button to hide:", reply_markup=button_visibility_keyboard("hide"))


@bot.message_handler(func=lambda m: m.text == "👁 Show Button" and is_admin(m.from_user.id))
def show_button_menu(m):
    bot.send_message(m.from_user.id, "👁 Select a user-menu button to show:", reply_markup=button_visibility_keyboard("show"))


@bot.callback_query_handler(func=lambda c: c.data.startswith("btnvis|"))
def button_visibility_callback(c):
    if not is_admin(c.from_user.id):
        bot.answer_callback_query(c.id, "Admin only", True)
        return
    _, mode, key = c.data.split("|", 2)
    if mode == "close":
        bot.delete_message(c.from_user.id, c.message.message_id)
        bot.answer_callback_query(c.id)
        return
    label = BUTTON_LABELS.get(key)
    if not label:
        bot.answer_callback_query(c.id, "Invalid button", True)
        return
    hidden = get_hidden_main_buttons()
    if mode == "hide":
        hidden.add(label)
        ...
        status = "shown"
    set_config("hidden_main_buttons", sorted(hidden))
    bot.answer_callback_query(c.id, f"{label} {status}", True)
    try:
        bot.edit_message_reply_markup(c.from_user.id, c.message.message_id, reply_markup=button_visibility_keyboard(mode))
    except Exception:
        pass
...
@bot.message_handler(func=lambda m: m.text == "🎯 Advanced Points" and is_admin(m.from_user.id))
def advanced_points_menu(m):
    bot.send_message(m.from_user.id,
        "🎯 **Advanced Points**\n\n"
        "`/pointadd USER_ID AMOUNT [note]`\n`/pointremove USER_ID AMOUNT [note]`\n"
        "`/pointclear USER_ID [note]`\n`/pointhistory USER_ID`\n`/topbuyers`", parse_mode="Markdown")

@bot.message_handler(commands=["pointadd", "pointremove", "pointclear", "pointhistory", "topbuyers"])
def advanced_point_commands(m):
    if not is_admin(m.from_user.id): return
    try:
        parts=m.text.split(maxsplit=3); cmd=parts[0][1:].lower()
        if cmd == "topbuyers":
            rows=list(users_col.find({}).sort("total_points_spent", -1).limit(30))
            text="🛒 **Top Buyers**\n\n"+"\n".join(f"{i}. `{u['_id']}` — {u.get('total_points_spent',0):,}" for i,u in enumerate(rows,1))
            return bot.send_message(m.from_user.id,text or "No data",parse_mode="Markdown")
        uid=parts[1]
        if cmd == "pointhistory":
            rows=list(point_history_col.find({"user_id":str(uid)}).sort("created_at",-1).limit(30))
            text="🧾 **Point History**\n\n"+"\n".join(f"{datetime.fromtimestamp(x['created_at']).strftime('%Y-%m-%d %H:%M')} | {x['amount']:+} | {x.get('reason','')}" for x in rows)
            return bot.send_message(m.from_user.id,text or "No history",parse_mode="Markdown")
        note=parts[3] if len(parts)>3 else None
        if cmd == "pointclear":
            old=users_col.find_one({"_id":str(uid)}) or {}; amount=-int(old.get("points",0))
            users_col.update_one({"_id":str(uid)},{"$set":{"points":0}}); add_point_history(uid,amount,"remove_all",m.from_user.id,note)
        else:
            amount=int(parts[2]); amount=amount if cmd=="pointadd" else -amount
            if not atomic_adjust_points(uid,amount,cmd,m.from_user.id,note): raise ValueError("User not found or insufficient points")
        done(m.from_user.id)
    except Exception as exc: bot.send_message(m.from_user.id,f"❌ {exc}")

@bot.message_handler(func=lambda m: m.text == "🔎 Search" and is_admin(m.from_user.id))
def search_menu(m):
    msg=bot.send_message(m.from_user.id,"🔎 Send user ID, username, name, payment ID, referral ID, or method name:")
    bot.register_next_step_handler(msg, search_process)

def search_process(m):
    if not is_admin(m.from_user.id): return
    q=(m.text or "").strip(); rx=re.compile(re.escape(q),re.I)
    ors=[{"_id":q},{"username":rx},{"name":rx},{"ref":q}]
    user=users_col.find_one({"$or":ors})
    payments=list(payments_col.find({"$or":[{"user_id":q},{"transaction_id":rx},{"status":rx}]}).limit(10))
    folders=list(folders_col.find({"name":rx}).limit(10))
    text="🔎 **Search Results**\n\n"
    if user: text+=f"👤 `{user['_id']}` @{user.get('username') or '-'}\nVIP: {user.get('vip',False)} | Points: {user.get('points',0)} | Ref: {user.get('ref') or '-'}\n\n"
    if payments: text+="💳 Payments: "+", ".join(str(x.get('_id')) for x in payments)+"\n"
    if folders: text+="📚 Methods: "+", ".join(x.get('name','') for x in folders)+"\n"
    bot.send_message(m.from_user.id,text if user or payments or folders else "No results",parse_mode="Markdown")

@bot.message_handler(func=lambda m: m.text == "🧾 Logs" and is_admin(m.from_user.id))
def view_logs(m):
    rows=list(logs_col.find({}).sort("created_at",-1).limit(50))
    text="🧾 **Latest Logs**\n\n"+"\n".join(f"{datetime.fromtimestamp(x['created_at']).strftime('%m-%d %H:%M')} | {x.get('action')} | {x.get('actor') or '-'}" for x in rows)
    bot.send_message(m.from_user.id,text or "No logs",parse_mode="Markdown")

@bot.message_handler(func=lambda m: m.text == "🧰 Folder Tools" and is_admin(m.from_user.id))
def folder_tools(m):
    bot.send_message(m.from_user.id,
      "🧰 **Folder Tools**\n\n`/folderduplicate NUMBER [new name]`\n`/foldermerge SOURCE_NUMBER TARGET_NUMBER`\n"
      "`/folderrepair` — numbering, missing parents and child links",parse_mode="Markdown")

@bot.message_handler(commands=["folderduplicate","foldermerge","folderrepair"])
def folder_tool_commands(m):
    if not is_admin(m.from_user.id): return
    try:
        p=m.text.split(maxsplit=2); cmd=p[0][1:]
        if cmd=="folderduplicate":
            src=folders_col.find_one({"number":int(p[1])});
            if not src: raise ValueError("Folder not found")
            src.pop("_id",None); src["name"]=(p[2] if len(p)>2 else src["name"]+" Copy"); src["number"]=get_config().get("next_folder_number",1); set_config("next_folder_number",src["number"]+1); folders_col.insert_one(src)
        elif cmd=="foldermerge":
            a=folders_col.find_one({"number":int(p[1])}); b=folders_col.find_one({"number":int(p[2])})
            if not a or not b: raise ValueError("Folder not found")
            merged=(b.get("files",[])+a.get("files",[])); text="\n\n".join(x for x in [b.get("text_content",""),a.get("text_content","")] if x)
            folders_col.update_one({"_id":b["_id"]},{"$set":{"files":merged,"text_content":text}}); folders_col.delete_one({"_id":a["_id"]})
        else:
            rows=list(folders_col.find({}).sort([("created_at",1),("_id",1)])); names={x.get("name") for x in rows}
            for i,x in enumerate(rows,1):
                upd={"number":i};
                if x.get("parent") and x.get("parent") not in names: upd["parent"]=None
                folders_col.update_one({"_id":x["_id"]},{"$set":upd})
            set_config("next_folder_number",len(rows)+1)
        log_event(cmd,m.from_user.id); done(m.from_user.id)
    except Exception as exc: bot.send_message(m.from_user.id,f"❌ {exc}")
...
@bot.message_handler(func=lambda m: m.text == "🎟 Promo Codes+" and is_admin(m.from_user.id))
def promo_plus(m):
    bot.send_message(m.from_user.id,"🎟 **Advanced Promo Codes**\n\n`/promogen POINTS COUNT MAX_USES EXPIRY_DAYS [vip|free|all]`\nExample: `/promogen 100 20 1 7 all`",parse_mode="Markdown")

@bot.message_handler(commands=['promogen'])
def promogen(m):
    if not is_admin(m.from_user.id): return
    try:
        _,pts,count,maxuses,days,*scope=m.text.split(); made=[]
        for _ in range(int(count)):
            code='ZEDOX'+''.join(random.choices(string.ascii_uppercase+string.digits,k=10))
            codes_col.insert_one({'_id':code,'points':int(pts),'used':False,'multi_use':int(maxuses)!=1,'used_count':0,'max_uses':int(maxuses),'expiry':now_ts()+int(days)*86400 if int(days)>0 else None,'created_at':now_ts(),'used_by_users':[],'scope':scope[0] if scope else 'all'})
            made.append(code)
        bot.send_message(m.from_user.id,"✅ Done Successfully\n\n"+'\n'.join(f'`{x}`' for x in made),parse_mode='Markdown')
    except Exception as exc: bot.send_message(m.from_user.id,f"❌ {exc}")

@bot.message_handler(func=lambda m: m.text == "📨 Advanced Broadcast" and is_admin(m.from_user.id))
def advanced_broadcast(m):
    bot.send_message(m.from_user.id,"📨 **Advanced Broadcast**\n\n`/bcschedule TARGET YYYY-MM-DD HH:MM` then send the post.\nTargets: all, vip, free\n`/bclist` | `/bcdelete ID`",parse_mode="Markdown")

_pending_bc={}
@bot.message_handler(commands=['bcschedule','bclist','bcdelete'])
def broadcast_schedule_commands(m):
    if not is_admin(m.from_user.id): return
    try:
        cmd=m.text.split()[0][1:]
        if cmd=='bclist':
            rows=list(broadcasts_col.find({}).sort('run_at',1).limit(30)); return bot.send_message(m.from_user.id,'\n'.join(f"`{x['_id']}` {x.get('target')} {datetime.fromtimestamp(x.get('run_at',0)).strftime('%Y-%m-%d %H:%M')} {x.get('status')}" for x in rows) or 'None',parse_mode='Markdown')
        if cmd=='bcdelete': broadcasts_col.delete_one({'_id':__import__('bson').ObjectId(m.text.split()[1])}); return done(m.from_user.id)
        _,target,date_s,time_s=m.text.split(); run=datetime.strptime(date_s+' '+time_s,'%Y-%m-%d %H:%M').timestamp(); _pending_bc[m.from_user.id]={'target':target,'run_at':run}; msg=bot.send_message(m.from_user.id,'Send/copy the post now:'); bot.register_next_step_handler(msg,save_scheduled_broadcast)
    except Exception as exc: bot.send_message(m.from_user.id,f"❌ {exc}")

def save_scheduled_broadcast(m):
    p=_pending_bc.pop(m.from_user.id,None)
    if not p:return
    broadcasts_col.insert_one({**p,'source_chat':m.chat.id,'source_message':m.message_id,'status':'scheduled','created_by':m.from_user.id,'created_at':now_ts()}); done(m.from_user.id)

@bot.message_handler(func=lambda m: m.text == "📣 Auto Posts" and is_admin(m.from_user.id))
def auto_posts_menu(m):
    bot.send_message(m.from_user.id,"📣 **Auto Posts**\n\n`/autopost CHANNEL every_hours HOURS` then send post\n`/autopost CHANNEL daily HH:MM`\n`/autolist` ` /autopause ID` ` /autoresume ID` ` /autodelete ID`",parse_mode='Markdown')

_pending_auto={}
@bot.message_handler(commands=['autopost','autolist','autopause','autoresume','autodelete'])
def auto_post_commands(m):
    if not is_admin(m.from_user.id):return
    try:
        a=m.text.split(); cmd=a[0][1:]
        if cmd=='autolist':
            rows=list(auto_posts_col.find({}).limit(30)); return bot.send_message(m.from_user.id,'\n'.join(f"`{x['_id']}` {x.get('channel')} {x.get('schedule')} {'ON' if x.get('active') else 'OFF'}" for x in rows) or 'None',parse_mode='Markdown')
        oid=__import__('bson').ObjectId(a[1]) if cmd!='autopost' else None
        if cmd in ('autopause','autoresume'): auto_posts_col.update_one({'_id':oid},{'$set':{'active':cmd=='autoresume'}}); return done(m.from_user.id)
        if cmd=='autodelete': auto_posts_col.delete_one({'_id':oid}); return done(m.from_user.id)
        if len(a) != 4:
            raise ValueError("Use /autopost CHANNEL every_hours HOURS or /autopost CHANNEL daily HH:MM")
        channel, mode, value = a[1], a[2].lower(), a[3]
        if mode == "every_hours":
            hours = float(value)
            if hours <= 0:
            ...
            raise ValueError("Schedule must be every_hours or daily")
        _pending_auto[m.from_user.id] = {'channel': channel, 'schedule': mode, 'value': value}
        msg = bot.send_message(m.from_user.id, 'Send or forward the post content now:')
...
def save_auto_post(m):
    ...
    auto_posts_col.insert_one({**x,'source_chat':m.chat.id,'source_message':m.message_id,'next_run':next_run,'active':True,'created_at':now}); done(m.from_user.id)

@bot.message_handler(func=lambda m: m.text == "📥 Auto Import" and is_admin(m.from_user.id))
def auto_import_menu(m):
    bot.send_message(m.from_user.id,"📥 **Auto Method Import**\n\n`/sourceadd CHAT_ID_OR_@USERNAME`\n`/sourceremove CHAT_ID_OR_@USERNAME`\n`/sourcelist`\n\nForward a real method to the bot and use `/importmethod CATEGORY PRICE` as a reply.",parse_mode='Markdown')

@bot.message_handler(commands=['sourceadd','sourceremove','sourcelist','importmethod'])
def source_commands(m):
    if not is_admin(m.from_user.id):return
    try:
        cmd=m.text.split()[0][1:]
        if cmd=='sourcelist':
            rows=list(source_chats_col.find({})); return bot.send_message(m.from_user.id,'\n'.join(str(x['_id']) for x in rows) or 'None')
        if cmd in ('sourceadd','sourceremove'):
            chat=m.text.split(maxsplit=1)[1]; (source_chats_col.update_one({'_id':chat},{'$set':{'active':True,'added_at':now_ts()}},upsert=True) if cmd=='sourceadd' else source_chats_col.delete_one({'_id':chat})); return done(m.from_user.id)
        if not m.reply_to_message: raise ValueError('Reply to a method message')
        _,cat,price=m.text.split(); src=m.reply_to_message; name=((src.text or src.caption or 'Imported Method').strip().splitlines()[0][:100]); files=[{'chat':src.chat.id,'msg':src.message_id,'type':src.content_type}]; number=fs.add(cat,name,files,int(price)); log_event('method_imported',m.from_user.id,number,{'name':name}); done(m.from_user.id,f'#{number} {name}')
    except Exception as exc: bot.send_message(m.from_user.id,f"❌ {exc}")
    ...
    while not _scheduler_stop.wait(20):
        try:
            ...
            for x in auto_posts_col.find({'active':True,'next_run':{'$lte':now}}).limit(20):
                try: bot.copy_message(x['channel'],x['source_chat'],x['source_message']); log_event('auto_post_sent',target=x['channel'])
                except Exception as exc: log_event('auto_post_error',target=x['channel'],details={'error':str(exc)},level='error')
                if x['schedule']=='every_hours': nxt=now+float(x['value'])*3600
                else: nxt=now+86400
                auto_posts_col.update_one({'_id':x['_id']},{'$set':{'next_run':nxt}})
...
def fallback(m):
    ...
    known = MAIN_MENU_BUTTONS + ["⚙️ ADMIN PANEL", "🔎 Search", "📣 Auto Posts", "📥 Auto Import", "🧾 Logs", "💾 Backup/Export", "🙈 Hide Button", "👁 Show Button"]
