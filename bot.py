...
class FS:
    ...
    def get(self, cat, parent=None):
        ...
        if parent:
            ...
            query["parent"] = None
        return list(folders_col.find(query).sort([("pinned", -1), ("sort_priority", 1), ("number", 1)]))
...
def methods_list_cmd(m):
    ...
    for cat, title, icon in categories:
        ...
        for index, row in enumerate(rows, 1):
            ...
            lines.append(f"{icon} {index:02d}. {pin}{row.get('name', 'Unnamed Method')}")
        sections.append("\n".join(lines))
    ...
    while text:
        ...
        if cut < 100:
            cut = 4096
        bot.send_message(m.from_user.id, text[:cut], parse_mode="Markdown")
...
def _send_payload(target, payload):
    ...
    if typ == "text":
        ...
        for index, chunk in enumerate(chunks):
            ...
            sent_ids.append(bot.send_message(target, chunk, reply_markup=markup).message_id)
    elif typ == "photo":
        sent_ids.append(bot.send_photo(target, payload["file_id"], caption=payload.get("caption") or None, reply_markup=reply_markup).message_id)
    elif typ == "video":
        sent_ids.append(bot.send_video(target, payload["file_id"], caption=payload.get("caption") or None, reply_markup=reply_markup).message_id)
    elif typ == "document":
        sent_ids.append(bot.send_document(target, payload["file_id"], caption=payload.get("caption") or None, reply_markup=reply_markup).message_id)
    elif typ == "animation":
        sent_ids.append(bot.send_animation(target, payload["file_id"], caption=payload.get("caption") or None, reply_markup=reply_markup).message_id)
    else:
        copied = bot.copy_message(target, payload["source_chat"], payload["source_message"])
        sent_ids.append(copied.message_id)
        if reply_markup:
            try:
                bot.edit_message_reply_markup(target, copied.message_id, reply_markup=reply_markup)
            except Exception:
                pass
    return sent_ids
...
@bot.message_handler(func=lambda m: m.chat.type in ("group", "supergroup"), content_types=['text','photo','video','document','audio','animation','voice'])
@bot.channel_post_handler(content_types=['text','photo','video','document','audio','animation','voice'])
def auto_import_channel_post(m):
    """Queue imported methods for approval; never publish directly."""
    try:
        ...
        cfg = get_cached_config()
        source_map = {
            cfg.get("auto_import_free_source"): "free",
            cfg.get("auto_import_vip_source"): "vip",
        }
        ...
        method_key = int(match.group(1))
        part = int(match.group(2) or 0)
        file_item = {"chat": m.chat.id, "msg": m.message_id, "type": m.content_type}

        if part:
            pending = pending_methods_col.find_one({"cat": cat, "auto_method_key": method_key, "status": "pending"})
            if pending:
                pending_methods_col.update_one(
                    {"_id": pending["_id"]},
                    {"$push": {"files": file_item}, "$set": {"updated_at": now_ts()}},
                )
                bot.send_message(m.chat.id, f"✅ File {part} added to pending #method{method_key}. Waiting for admin approval.")
                return

            approved = folders_col.find_one({"cat": cat, "auto_method_key": method_key})
            if approved:
                # Updates also require approval rather than changing live content immediately.
                pending_methods_col.update_one(
                    {"cat": cat, "auto_method_key": method_key, "status": "pending_update"},
                    {"$setOnInsert": {
                        "cat": cat, "auto_method_key": method_key, "name": approved.get("name"),
                        "source_chat": m.chat.id, "created_at": now_ts(), "status": "pending_update",
                        "existing_folder_id": approved.get("_id"), "submitted_by": getattr(m.from_user, "id", None),
                    }, "$push": {"files": file_item}, "$set": {"updated_at": now_ts()}},
                    upsert=True,
                )
                bot.send_message(m.chat.id, f"✅ Update file {part} is pending admin approval for {approved.get('name')}.")
                return

            bot.send_message(m.chat.id, f"❌ #method{method_key} has no pending or approved main method yet.")
            return

        lines = raw.splitlines()
        name = (lines[1].strip() if len(lines) > 1 and lines[1].strip() else f"Method {method_key}")[:100]
        existing_pending = pending_methods_col.find_one({"cat": cat, "auto_method_key": method_key, "status": "pending"})
        existing_live = folders_col.find_one({"cat": cat, "auto_method_key": method_key})

        if existing_pending:
            pending_methods_col.update_one(
                {"_id": existing_pending["_id"]},
                {"$set": {"name": name, "files": [file_item], "updated_at": now_ts()}},
            )
            pending_id = existing_pending["_id"]
            action = "updated in pending queue"
        elif existing_live:
            doc = {
                "cat": cat, "auto_method_key": method_key, "name": name, "files": [file_item],
                "source_chat": m.chat.id, "created_at": now_ts(), "updated_at": now_ts(),
                "status": "pending_replace", "existing_folder_id": existing_live.get("_id"),
                "submitted_by": getattr(m.from_user, "id", None),
            }
            ...
            doc = {
                "cat": cat, "auto_method_key": method_key, "name": name, "files": [file_item],
                "source_chat": m.chat.id, "created_at": now_ts(), "updated_at": now_ts(),
                "status": "pending", "submitted_by": getattr(m.from_user, "id", None),
            }
            pending_id = pending_methods_col.insert_one(doc).inserted_id
            action = "sent for approval"

        bot.send_message(m.chat.id, f"⏳ {name} has been {action}. It is not visible in the bot yet.")
        try:
            kb = InlineKeyboardMarkup(row_width=2)
            kb.add(
            ...
            bot.send_message(ADMIN_ID, f"⏳ **Pending Method**\n\n📁 {cat.upper()}\n🏷 {name}\n🔢 Source: #method{method_key}\n📎 Files: 1", reply_markup=kb, parse_mode="Markdown")
        except Exception:
            pass
    except Exception as exc:
        try:
            bot.send_message(m.chat.id, f"❌ Auto import failed: {str(exc)[:500]}")
        except Exception:
            pass
...
def pending_methods_menu(m):
    ...
    bot.send_message(m.from_user.id, f"⏳ **PENDING METHODS**\n\nWaiting for review: **{count}**\n\nSelect a method:", reply_markup=kb, parse_mode="Markdown")
...
def pending_page_cb(c):
    ...
    bot.edit_message_text(f"⏳ **PENDING METHODS**\n\nWaiting for review: **{count}**\n\nSelect a method:", c.from_user.id, c.message.message_id, reply_markup=kb, parse_mode="Markdown")
    bot.answer_callback_query(c.id)
...
def pending_view_cb(c):
    ...
    try:
        ...
        bot.send_message(c.from_user.id, text, reply_markup=kb, parse_mode="Markdown")
        bot.answer_callback_query(c.id)
    except Exception as exc:
...
def pending_approve_cb(c):
    ...
    msg = bot.send_message(c.from_user.id, "💰 Send the price in points for this method.\n\nSend `0` for a free method.", parse_mode="Markdown")
...
def pending_price_step(m):
    ...
    try:
        ...
        bot.send_message(m.from_user.id, f"✅ **Process Complete**\n\n{row.get('name')} has been {action}.\n💰 Price: **{price} points**\n📌 It appears below pinned methods and above older unpinned methods.", reply_markup=admin_menu(), parse_mode="Markdown")
        try:
            bot.send_message(row.get("source_chat"), f"✅ {row.get('name')} was approved and published at {price} points.")
        except Exception:
            pass
    except Exception as exc:
...
def pending_reject_cb(c):
    ...
    try:
        ...
        bot.send_message(c.from_user.id, f"✅ Process Complete\nRejected: {row.get('name')}", reply_markup=admin_menu())
        try:
            bot.send_message(row.get("source_chat"), f"❌ {row.get('name')} was rejected by admin and was not published.")
        except Exception:
            pass
    except Exception as exc:
...
def pin_methods_keyboard(action, page=0, page_size=12):
    ...
    for row in rows[page * page_size:(page + 1) * page_size]:
        ...
        kb.add(InlineKeyboardButton(f"{icon} ({row.get('number')}) {row.get('name')}", callback_data=f"methodpinset|{action}|{row['_id']}"))
    nav = []
    if page > 0:
...
def pin_methods_menu(m):
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("📌 Pin a Method", callback_data="methodpinmenu|pin"), InlineKeyboardButton("📍 Unpin a Method", callback_data="methodpinmenu|unpin"))
    bot.send_message(m.from_user.id, "📌 **METHOD PLACEMENT**\n\nPinned methods always appear first. Newly approved methods appear directly below pinned methods.", reply_markup=kb, parse_mode="Markdown")
...
def fallback(m):
    ...
    known = MAIN_MENU_BUTTONS + [
        ...
        "🧾 Logs", "💾 Backup/Export", "🙈 Hide Button", "👁 Show Button", "📋 METHODS LIST"
    ]
