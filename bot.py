...
def method_status_cb(c):
    ...
    if not row: return bot.answer_callback_query(c.id,"Not found",True)
    expired = mode == "expire"
    result = folders_col.update_one({"_id": row["_id"]}, {"$set": {"expired": expired, "updated_at": now_ts()}})
    if result.matched_count != 1:
        return bot.answer_callback_query(c.id, "Update failed", True)
    ...
    try:
        ...
        if part:
            ...
            if pending:
                ...
                if cfg.get("group_import_notify_enabled", True):
            ...
            if cfg.get("group_import_notify_enabled", True):
                raw_bot.send_message(m.chat.id, f"❌ #method{method_key} has no pending or approved main method yet.")
            return
