...
bot = telebot.TeleBot(BOT_TOKEN, parse_mode="Markdown", threaded=True, num_threads=4)
...
class FS:
    def add(self, cat, name, files, price, parent=None, number=None, text_content=None):
        ...
        # Separate visible numbering for each category (FREE/VIP/APPS/SERVICES).
        last_cat = folders_col.find_one({"cat": cat}, sort=[("cat_number", -1)])
        cat_number = int((last_cat or {}).get("cat_number", 0)) + 1
        folder_data = {
            "cat": cat,
            "name": name,
            "files": files,
            "price": price,
            "parent": parent,
            "number": number,              # legacy global ID, kept for compatibility
            "cat_number": cat_number,      # visible per-category number
            "created_at": time.time()
        }
...
def ensure_category_numbers():
    """Give old records a stable number inside their own category."""
    for cat in ("free", "vip", "apps", "services"):
        docs = list(folders_col.find({"cat": cat}).sort([("number", 1), ("created_at", 1)]))
        used = set()
        next_no = 1
        for doc in docs:
            current = doc.get("cat_number")
            if isinstance(current, int) and current > 0 and current not in used:
                used.add(current)
                next_no = max(next_no, current + 1)
                continue
            while next_no in used:
                next_no += 1
            folders_col.update_one({"_id": doc["_id"]}, {"$set": {"cat_number": next_no}})
            used.add(next_no)
            next_no += 1

ensure_category_numbers()
...
# ✅ VERIFIED REFERRALS
# =========================
def complete_pending_referral(uid):
    """Count a referral only after the new user passes every force-join check."""
    user_doc = users_col.find_one({"_id": str(uid)}) or {}
    ref_id = user_doc.get("pending_ref")
    if not ref_id or user_doc.get("ref"):
        return False
    if ref_id == str(uid) or not users_col.find_one({"_id": str(ref_id)}):
        users_col.update_one({"_id": str(uid)}, {"$unset": {"pending_ref": ""}})
        return False

    # Atomic claim prevents double rewards when /start and Recheck happen together.
    claimed = users_col.update_one(
        {"_id": str(uid), "ref": None, "pending_ref": str(ref_id)},
        {"$set": {"ref": str(ref_id)}, "$unset": {"pending_ref": ""}}
    )
    if claimed.modified_count != 1:
        return False

    User._cache.pop(uid, None)
    ...
    try:
        vip_msg = ""
        if got_vip:
            vip_msg = f"\n\n🎉 **CONGRATULATIONS!**\nYou reached {ref_user.get_refs_count()} referrals and received FREE VIP."
        bot.send_message(
            int(ref_id),
            f"👤 **Verified Referral!**\n\nA user joined every required channel/group.\n"
        ...
        print(f"Referral notification failed: {exc}", flush=True)
    return True

# =========================
# 🚫 FORCE JOIN (CHANNELS + GROUPS + LINKS)
# =========================
_force_cache = {}
FORCE_CACHE_TTL = 10

def normalize_force_entry(entry):
    # Backward compatibility with old @username strings.
    if isinstance(entry, str):
        raw = entry.strip()
        if raw.startswith("@"): return {"chat_id": raw, "url": f"https://t.me/{raw[1:]}", "title": raw}
        if raw.startswith("https://t.me/") and "+" not in raw:
            username = raw.rstrip("/").split("/")[-1]
            return {"chat_id": f"@{username}", "url": raw, "title": f"@{username}"}
        return {"chat_id": raw, "url": raw if raw.startswith("http") else "", "title": raw}
    if isinstance(entry, dict):
        return {
            "chat_id": entry.get("chat_id") or entry.get("username"),
            "url": entry.get("url") or entry.get("link") or "",
            "title": entry.get("title") or entry.get("name") or str(entry.get("chat_id") or "Join"),
        }
    return {"chat_id": None, "url": "", "title": "Join"}

def force_entries():
    return [normalize_force_entry(x) for x in get_cached_config().get("force_channels", [])]

def force_join_keyboard(entries):
    kb = InlineKeyboardMarkup(row_width=1)
    for entry in entries:
        if entry.get("url"):
            kb.add(InlineKeyboardButton(f"📢 Join {entry['title']}", url=entry["url"]))
    kb.add(InlineKeyboardButton("✅ I Joined — Verify", callback_data="recheck"))
    return kb
...
def force_block(uid):
    ...
    for entry in entries:
        ...
        try:
            ...
            print(f"Force-join check failed for {chat_id}: {exc}", flush=True)
            missing.append(entry)
    if missing:
        bot.send_message(
            uid,
            "🚫 **Access Restricted**\n\nJoin every required channel/group, then press **I Joined — Verify**.",
            reply_markup=force_join_keyboard(entries),
            parse_mode="Markdown"
        )
        return True
    return False
...
def force_join_handler(func):
    ...
    def wrapper(message):
        if force_block(message.from_user.id):
            return
        complete_pending_referral(message.from_user.id)
        return func(message)
    return wrapper
...
def start_cmd(m):
    ...
    if len(args) > 1:
        ...
        if ref_id != str(uid) and ref_id.isdigit() and not user.data.get("ref"):
            if users_col.find_one({"_id": ref_id}):
...
def get_folders_kb(cat, parent=None, page=0, items_per_page=15):
    ...
    for item in page_items:
        ...
        number = item.get("cat_number", item.get("number", "?"))
...
def open_folder(c):
    ...
    if subfolders and len(subfolders) > 0:
        ...
        for sub in subfolders:
            sub_name = sub["name"]
            sub_number = sub.get("cat_number", sub.get("number", "?"))
    ...
    # Direct Telegram bot, product, website or custom service link.
    service_url = folder.get("service_url") or folder.get("bot_url")
    if cat == "services" and service_url:
        kind = folder.get("service_kind", "bot" if folder.get("bot_url") else "link")
        labels = {"bot": "🚀 Open Bot", "product": "🛍 Open Product", "link": "🔗 Open Link"}
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton(labels.get(kind, "🔗 Open"), url=service_url))
        service_msg = folder.get("service_msg") or folder.get("text_content") or f"Open **{name}** using the button below."
        bot.send_message(uid, f"⚡ **{name}**\n\n{service_msg}", reply_markup=kb, parse_mode="Markdown")
...
def get_points_callback(c):
    fake_message = type("PointMessage", (), {"from_user": c.from_user})()
    get_points_button(fake_message)
    bot.answer_callback_query(c.id, "✅ Points menu opened")
...
def recheck(c):
    ...
    if not force_block(uid):
        ...
        try:
            ...
            pass
        bot.send_message(uid, f"✅ Verification done successfully!\n\n🎉 Welcome!\n💰 Points: {user.points()}", reply_markup=main_menu(uid))
        bot.answer_callback_query(c.id, "✅ Done")
    else:
        bot.answer_callback_query(c.id, "❌ Join all required channels/groups first!", True)
...
def show_purchased_methods(m):
    ...
    all_vip_methods = {item["name"]: item.get("cat_number", item.get("number", "?")) for item in fs.get("vip")}
...
def admin_menu():
    ...
    kb.row("💰 Give Points", "🧹 Remove All Points")
    ...
    kb.row("➖ Remove Button", "➕ Add Channel")
...
# =========================
@bot.message_handler(func=lambda m: m.text == "🧹 Remove All Points" and is_admin(m.from_user.id))
def remove_all_points_start(m):
    total_points = list(users_col.aggregate([{"$group": {"_id": None, "total": {"$sum": "$points"}}}]))
    total = total_points[0]["total"] if total_points else 0
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("✅ Remove From Everyone", callback_data="reset_all_points"), InlineKeyboardButton("❌ Cancel", callback_data="reset_points_cancel"))
    bot.send_message(m.from_user.id, f"⚠️ **Remove all current points?**\n\nTotal points to remove: **{total:,}**\nThis affects every user.", reply_markup=kb, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda c: c.data == "reset_all_points")
def remove_all_points_confirm(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    result = users_col.update_many({"points": {"$ne": 0}}, {"$set": {"points": 0}})
    User._cache.clear()
    User._cache_time.clear()
    bot.edit_message_text(f"✅ Points removed from {result.modified_count} users.", c.from_user.id, c.message.message_id)
    bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data == "reset_points_cancel")
def remove_all_points_cancel(c):
    bot.edit_message_text("❌ Cancelled. No points were changed.", c.from_user.id, c.message.message_id)
    bot.answer_callback_query(c.id)
...
def upload_type_choice(m, cat, is_service):
    ...
    elif m.text == "🤖 Direct Bot" and is_service:
        msg = bot.send_message(m.from_user.id, "🤖 Send bot username or link\nExample: `@mybot`", parse_mode="Markdown")
        bot.register_next_step_handler(msg, lambda x: service_bot_username(x, cat))
    elif m.text in ("🔗 Direct Link", "🛍 Product Link") and is_service:
        kind = "product" if m.text == "🛍 Product Link" else "link"
        msg = bot.send_message(m.from_user.id, "🔗 Send the complete URL, starting with https://")
        bot.register_next_step_handler(msg, lambda x: service_link_url(x, cat, kind))
    elif m.text == "📁 Files":
        kb = ReplyKeyboardMarkup(resize_keyboard=True)
        kb.row("/done", "/cancel")
...
def service_bot_username(m, cat):
    raw = (m.text or "").strip()
    if raw.startswith("@"):
        username = raw[1:]
        ...
        bot.send_message(m.from_user.id, "❌ Send @botusername or https://t.me/botusername", reply_markup=admin_menu())
        return
    msg = bot.send_message(m.from_user.id, "📝 Service button name:")
    bot.register_next_step_handler(msg, lambda x: service_bot_name(x, cat, bot_url))

def service_bot_name(m, cat, bot_url):
    name = (m.text or "").strip()
    msg = bot.send_message(m.from_user.id, "📝 Description shown above the Open Bot button:")
    bot.register_next_step_handler(msg, lambda x: service_bot_save(x, cat, name, bot_url))

def service_bot_save(m, cat, name, bot_url):
    description = (m.text or "").strip()
    number = fs.add(cat, name, [], 0, text_content=description)
    folder = fs.get_by_number(number)
    if folder:
        folders_col.update_one({"_id": folder["_id"]}, {"$set": {"bot_url": bot_url, "service_msg": description}})
    bot.send_message(m.from_user.id, f"✅ Direct bot service added!\n📌 #{number}\n🤖 {bot_url}", reply_markup=admin_menu())
    upload_sessions.pop(m.from_user.id, None)

def service_link_url(m, cat, kind):
    url = (m.text or "").strip()
    if not url.startswith(("https://", "http://")):
        bot.send_message(m.from_user.id, "❌ Invalid URL. It must start with https://", reply_markup=admin_menu())
        return
    msg = bot.send_message(m.from_user.id, "📝 Send the product/service name:")
    bot.register_next_step_handler(msg, lambda x: service_link_name(x, cat, kind, url))

def service_link_name(m, cat, kind, url):
    name = (m.text or "").strip()
    if not name:
        bot.send_message(m.from_user.id, "❌ Name cannot be empty.", reply_markup=admin_menu())
        return
    msg = bot.send_message(m.from_user.id, "📝 Send the description:")
    bot.register_next_step_handler(msg, lambda x: service_link_save(x, cat, kind, url, name))

def service_link_save(m, cat, kind, url, name):
    description = (m.text or "").strip()
    global_number = fs.add(cat, name, [], 0, text_content=description)
    folder = fs.get_by_number(global_number)
    visible_number = global_number
    if folder:
        visible_number = folder.get("cat_number", global_number)
        folders_col.update_one({"_id": folder["_id"]}, {"$set": {
            "service_url": url, "service_kind": kind, "service_msg": description
        }})
    label = "Product" if kind == "product" else "Direct link"
    bot.send_message(m.from_user.id, f"✅ {label} added successfully!\n📌 #{visible_number}\n🔗 {url}", reply_markup=admin_menu())
    upload_sessions.pop(m.from_user.id, None)
...
def del_start(m):
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton("📂 Free", callback_data="delcat|free"),
        InlineKeyboardButton("💎 VIP", callback_data="delcat|vip"),
        InlineKeyboardButton("📱 Apps", callback_data="delcat|apps"),
        InlineKeyboardButton("⚡ Services", callback_data="delcat|services")
    )
    bot.send_message(m.from_user.id, "🗑 **Delete Method/Folder**\n\nChoose a category:", reply_markup=kb, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda c: c.data.startswith("delcat|"))
def delete_category_buttons(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    cat = c.data.split("|", 1)[1]
    folders = list(folders_col.find({"cat": cat}).sort("number", 1).limit(90))
    kb = InlineKeyboardMarkup(row_width=1)
    for folder in folders:
        number = folder.get("number")
        parent = f" / {folder.get('parent')}" if folder.get("parent") else ""
        kb.add(InlineKeyboardButton(
            f"🗑 #{number} {folder.get('name')}{parent}",
            callback_data=f"delnum|{number}"
        ))
    kb.add(InlineKeyboardButton("🔙 Categories", callback_data="delback"))
    text = f"🗑 **{cat.upper()}**\nSelect what to delete:"
    if not folders:
        text += "\n\nNo methods or folders found."
    bot.edit_message_text(text, c.from_user.id, c.message.message_id, reply_markup=kb, parse_mode="Markdown")
    bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data == "delback")
def delete_back(c):
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton("📂 Free", callback_data="delcat|free"),
    ...
    bot.edit_message_text("🗑 Choose a category:", c.from_user.id, c.message.message_id, reply_markup=kb)
    bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("delnum|"))
def delete_number_confirm(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    number = int(c.data.split("|", 1)[1])
    folder = fs.get_by_number(number)
    if not folder:
        return bot.answer_callback_query(c.id, "Already deleted", True)
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton("✅ Delete Now", callback_data=f"delok|{number}"),
        InlineKeyboardButton("❌ Cancel", callback_data=f"delcat|{folder['cat']}")
    )
    sub_count = folders_col.count_documents({"cat": folder["cat"], "parent": folder["name"]})
    bot.edit_message_text(
...
def delete_number_execute(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    number = int(c.data.split("|", 1)[1])
    folder = fs.get_by_number(number)
    if folder and fs.delete(folder["cat"], folder["name"], folder.get("parent")):
        bot.edit_message_text(f"✅ Deleted: {folder['name']}", c.from_user.id, c.message.message_id)
    else:
        bot.edit_message_text("❌ Item not found.", c.from_user.id, c.message.message_id)
    bot.answer_callback_query(c.id)
...
def add_channel_cmd(m):
    text = (
        ...
        "Public channel/group: send `@username` or `https://t.me/username`\n\n"
        "Private invite link: send exactly:\n"
        "`invite_link | chat_id | display name`\n\n"
        "Example:\n`https://t.me/+AbCd123 | -1001234567890 | VIP Group`\n\n"
        "The bot must be an admin/member of that channel or group."
    )
    msg = bot.send_message(m.from_user.id, text, parse_mode="Markdown")
...
def add_channel_process(m):
    ...
    if "|" in raw:
        ...
        if len(parts) == 3 and parts[0].startswith("https://t.me/"):
            try:
                chat_id = int(parts[1])
                entry = {"url": parts[0], "chat_id": chat_id, "title": parts[2] or "Group"}
            except ValueError:
                pass
    elif raw.startswith("@"):
        entry = {"url": f"https://t.me/{raw[1:]}", "chat_id": raw, "title": raw}
    elif raw.startswith("https://t.me/") and "+" not in raw:
        username = raw.rstrip("/").split("/")[-1]
        entry = {"url": raw, "chat_id": f"@{username}", "title": f"@{username}"}
    if not entry:
        bot.send_message(m.from_user.id, "❌ Invalid format. Private links require link | chat_id | name.", reply_markup=admin_menu())
        return
    try:
        bot.get_chat(entry["chat_id"])
    except Exception as exc:
        bot.send_message(m.from_user.id, f"❌ I cannot access that chat. Add the bot there first.\n\n{exc}", reply_markup=admin_menu())
        return
    cfg = get_config(); entries = cfg.get("force_channels", [])
    normalized_ids = {str(normalize_force_entry(x).get("chat_id")) for x in entries}
    if str(entry["chat_id"]) in normalized_ids:
        bot.send_message(m.from_user.id, "❌ Already added!", reply_markup=admin_menu()); return
    entries.append(entry); set_config("force_channels", entries)
    bot.send_message(m.from_user.id, f"✅ Force-join added successfully!\n📢 {entry['title']}", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "➖ Remove Channel" and is_admin(m.from_user.id))
def remove_channel_cmd(m):
    entries = get_config().get("force_channels", [])
    if not entries:
        bot.send_message(m.from_user.id, "❌ No force-join chats!", reply_markup=admin_menu()); return
    kb = InlineKeyboardMarkup(row_width=1)
    for i, raw in enumerate(entries):
        entry = normalize_force_entry(raw)
        kb.add(InlineKeyboardButton(f"❌ {entry['title']}", callback_data=f"rmchidx_{i}"))
    bot.send_message(m.from_user.id, "Select channel/group to remove:", reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("rmchidx_"))
def remove_channel_cb(c):
    try:
        idx = int(c.data.split("_", 1)[1])
        cfg = get_config(); entries = cfg.get("force_channels", [])
        removed = normalize_force_entry(entries.pop(idx))
        set_config("force_channels", entries)
        bot.edit_message_text(f"✅ Removed successfully: {removed['title']}", c.from_user.id, c.message.message_id)
        bot.answer_callback_query(c.id, "✅ Done")
    except Exception:
        bot.answer_callback_query(c.id, "❌ Could not remove", True)
