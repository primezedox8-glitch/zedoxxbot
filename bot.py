...
class FS:
    ...
    def get(self, cat, parent=None):
        ...
        if parent:
            ...
            query["parent"] = None
        return list(folders_col.find(query).sort([("sort_priority", 1), ("number", 1)]))
...
# =========================
# 📋 METHODS LIST (NAMES ONLY)
# =========================
@bot.message_handler(func=lambda m: m.text == "📋 METHODS LIST")
@force_join_handler
def methods_list_cmd(m):
    labels = [("free", "📂 FREE METHODS"), ("vip", "💎 VIP METHODS"), ("apps", "📦 PREMIUM APPS"), ("services", "⚡ SERVICES")]
    sections = []
    for cat, title in labels:
        rows = fs.get(cat)
        names = [str(row.get("name", "")).strip() for row in rows if str(row.get("name", "")).strip()]
        if names:
            sections.append(title + "\n" + "\n".join(f"• {name}" for name in names))
    text = "📋 **METHODS LIST**\n\n" + ("\n\n".join(sections) if sections else "No methods available.")
    # Split safely for Telegram while preserving all names.
    while text:
        if len(text) <= 4096:
...
def auto_import_channel_post(m):
    try:
        ...
        if existing:
            ...
            number=fs.add(cat,name,[file_item],0,at_start=True);folders_col.update_one({'number':number},{'$set':{'auto_method_key':key,'source_chat':m.chat.id}})
